...
public class OverlayImageIcon extends CompositeImageDescriptor {
	 ...
	 * 
	 */
	protected void drawCompositeImage(int arg0, int location) {
		// Draw the base image
		drawImage(baseImage.getImageData(), 0, 0);
		int[] locations = organizeImages();
		for (int i = 0; i < imageKey.size(); i++) {
			ImageData imageData = ImageDescriptorManager
					.getImageData(ImageDescriptorManager.SHARE_WIZARD);
			switch (location) {
	 ...
	 * out the location of the images. Here we use only one image(with key
	 * "Lock")
	 */
