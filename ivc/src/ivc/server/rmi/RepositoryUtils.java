...
public class RepositoryUtils {
	...
	/**
	 * Creates all necessary files and folders for a project. The method is invoked at the
	 * time a new project is added.
	 * 
	 * @param projectPath
	 */
	public static void initRepository(String projectPath) {
		...
		File bvFile = new File(Constants.RepositoryFolder + projectPath
		...
		File cvFile = new File(Constants.RepositoryFolder + projectPath
		...
		File rclFile = new File(Constants.RepositoryFolder + projectPath
				+ Constants.PendingRemoteCommitedLog);
		try {
			rclFile.createNewFile();
		} catch (IOException e) {
			e.printStackTrace();
		}
		File transFile = new File(Constants.RepositoryFolder + projectPath
